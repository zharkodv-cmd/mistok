...
Слоти під картинки — порожні плейсхолдери за іменем (ph/img/image/photo/…) або листові ноди, де фото вже стоїть.

- **Кнопка Reuse** — підставляє у слоти найдоречніші фото, що вже є у файлі.
- **Кнопка ✨ Photos** — bridge бере справжні фото з публічного надбання на **Openverse** (CC0/PDM: StockSnap, Rawpixel, Flickr, Wikimedia; без ключа, без атрибуції, без AI-картинок): haiku перетворює тексти біля слота на короткі англійські запити, вставка ~1000 px. Анонімна квота Openverse — 20 запитів/хв, 200/день.
- **«встав картинки» у сесії** — `mistok sel` → слоти → фото на свій вибір (НЕ генеруй AI-картинки, якщо прямо не попросили) → кожне через **`mistok img <slotId> file.png`** (CLI сам кодує байти — ніколи не тягни base64 через контекст).
