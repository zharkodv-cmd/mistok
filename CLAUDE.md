...
## Award-дизайн з прототипу (кнопка ◆ Design)

Кнопка пише запит у `/tmp/mistok-design-request.json`. Коли користувач каже «build the design» / «зроби дизайн»: СПОЧАТКУ завантаж скіл `impeccable` (Skill tool), прочитай запит; збери повноцінний award-рівня дизайн ПОРУЧ з прототипом: variables файлу (скоупи!), text styles, наявні асети; brand-register, смілива направленість, без AI-слопу; 1 відполірований варіант + короткі design notes поряд. Після виконання видали файл запиту.
