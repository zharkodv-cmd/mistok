...
**Кнопка ✨ у плагіні** пише запит у `/tmp/mistok-image-request.json` (frame, слоти: id/розміри/сусідні тексти). Коли користувач каже «встав картинки» — прочитай запит, згенеруй через Magnific за правилами вище (контекст слота = сусідні тексти + арт-дирекшн проєкту), збережи PNG і встав кожен через **`mistok img <slotId> file.png`** (CLI сам кодує байти — ніколи не тягни base64 через контекст). Після виконання видали файл запиту.
