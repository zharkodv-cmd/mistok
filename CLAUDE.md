...
4. Референси — натхнення, не копія. Після виконання видали файл запиту.
...
Кнопка пише запит у `/tmp/mistok-design-request.json`. Коли користувач каже «recreate the design» / «відтвори дизайн»: джерело — скріншот (фрейм з IMAGE). `mistok shot` у scale 1–2, уважно прочитай; збери ПОРУЧ 1:1 редаговані шари: точні позиції/розміри, кольори семпловані з зображення (біндити до variables лише при точному/близькому збігу), справжні TEXT-ноди з відповідними розміром/вагою/міжряддям (шрифти проєкту або Inter), плашки/бордери/радіуси/тіні як на скріні. Наприкінці — порівняльний shot і виправлення дельт. Видали файл запиту.

## Redesign — доповнення

Джерелом може бути скріншот/набросок (фрейм з одним IMAGE-прямокутником) — тоді спершу `mistok shot` і прочитай візуально. Відступи/масштаб/композиція — З РЕФЕРЕНСУ (візуально, не копіюй паддінги джерела); з нашої системи — лише кольори/шрифти/асети. Фінальний прохід — крафт за impeccable.
